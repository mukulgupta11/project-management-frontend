...
const ViewTaskDetails = () => {
  ...
  const [todoCheckList, setTodoCheckList] = useState(task?.todoCheckList || []);
  const [newTodo, setNewTodo] = useState('');

  const handleAddTodo = () => {
  if (newTodo.trim()) {
    setTodoCheckList([...todoCheckList, { text: newTodo, completed: false }]);
    setNewTodo('');
   }
 };
 const handleToggleTodo = (index) => {
  const updated = [...todoCheckList];
...
const handleUpdateChecklist = async () => {
  try {
    const response = await axiosInstance.put(
      `/tasks/update-checklist/${task._id}`,
      { todoCheckList }
    );
 ...
 todoCheckList[index].completed = !todoCheckList[index].completed;
  ...
  const allCompleted = todoCheckList.every(item => item.completed);
      ...
      const response = await axiosInstance.put(API_PATHS.TASKS.UPDATE_TASK_CHECKLIST(taskId), {
        todoCheckList,
        status: allCompleted ? "Unverified" : task.status
      });
